...
class PairTradingStrategy:
    def __init__(self, symbols: list[str], _historical_data_dir: str, lookback_days: int):
    ...
    def _get_daily_closes(self, base_symbol: str) -> pd.Series:
        ...
        df = df[
            df["timestamp"].between(window_start, self.oms_client.current_time, inclusive="left")
        ]
    ...
    def run_strategy(self, oms_client: OMSClient, data_manager: HistoricalDataCollector):
        ...
        for p in self.pairs:
            ...
            if enter:
                ...
                day_key = (
                    self.oms_client.current_time.year,
                ...
                if self._last_entry_day.get(id(p)) == day_key:
                    ...
                    if z > 0:
                        ...
                        # Short A, Long B * beta
                        orders.append(
                            {"symbol": a_sym, "instrument_type": instrument_type, "side": "SHORT"}
                        )
                        orders.append(
                            {"symbol": b_sym, "instrument_type": instrument_type, "side": "LONG"}
                        )
                        ...
                        # Long A, Short B * beta
                        orders.append(
                            {"symbol": a_sym, "instrument_type": instrument_type, "side": "LONG"}
                        )
                        orders.append(
                            {"symbol": b_sym, "instrument_type": instrument_type, "side": "SHORT"}
                        )
                ...
                # Close both legs
                orders.append(
                    {"symbol": a_sym, "instrument_type": instrument_type, "side": "CLOSE"}
                )
                orders.append(
                    {"symbol": b_sym, "instrument_type": instrument_type, "side": "CLOSE"}
                )
                p["is_open"] = False
                p["current_side"] = None
                all_orders.extend(orders)
