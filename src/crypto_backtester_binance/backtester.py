...
from .format_utils import format_positions_table
...
class Backtester:
    ...
    def run_backtest(
        ...
        for sym in base_symbols:
            try:
                if market_type == "spot":
                    self.data_manager.load_data_period(
                        sym,
                        desired_timeframe,
                        "ohlcv_spot",
                        data_start_date,
        ...
        for sym in base_symbols:
            for store in [
                ...
                if df is not None and not df.empty:
                    ...
                    earliest_ts = (
                        first if earliest_ts is None or first < earliest_ts else earliest_ts
                    )
        ...
        while self.oms_client.current_time <= end_date:
            try:
                ...
                orders = strategy.run_strategy(
                    oms_client=self.oms_client, data_manager=self.data_manager
                )

                # we pass the oms and data manager to the position manager so it would have the most up to date information
                filtered_orders = self.position_manager.filter_orders(
                    orders=orders, oms_client=self.oms_client, data_manager=self.data_manager
                )
    ...
    def run_permutation_backtest(
        ...
        for i in range(permutations + 1):
            ...
            if i > 0:
                for sym in base_symbols:
                    ...
                    if market_type == "spot" and sym in orig_spot:
                        self.data_manager.spot_ohlcv_data[sym] = (
                            orig_spot[sym].sample(frac=1, random_state=i).reset_index(drop=True)
                        )
                        self.data_manager.spot_ohlcv_data[sym]["timestamp"] = orig_spot[sym][
                            "timestamp"
                        ]
                    elif market_type == "futures":
                        if sym in orig_index_future:
                            self.data_manager.perpetual_index_ohlcv_data[sym] = (
                                orig_index_future[sym]
                                .sample(frac=1, random_state=i)
                                .reset_index(drop=True)
                            )
                            self.data_manager.perpetual_index_ohlcv_data[sym]["timestamp"] = (
                                orig_index_future[sym]["timestamp"]
                            )
                        if sym in orig_mark_future:
                            self.data_manager.perpetual_mark_ohlcv_data[sym] = (
                                orig_mark_future[sym]
                                .sample(frac=1, random_state=i + 1337)
                                .reset_index(drop=True)
                            )
                            self.data_manager.perpetual_mark_ohlcv_data[sym]["timestamp"] = (
                                orig_mark_future[sym]["timestamp"]
                            )
                    else:
                        raise ValueError(f"Invalid market type: {market_type}")
            ...
            while self.oms_client.current_time <= end_date:
                try:
                    ...
                    filtered = self.position_manager.filter_orders(
                        orders=orders, oms_client=self.oms_client, data_manager=self.data_manager
                    )
        ...
        if len(self.permutation_returns) > 0:
            ...
            B = sharpes.size
            p_value = (
                (1 + np.sum(sharpes >= T_obs)) / (B + 1) if B > 0 and np.isfinite(T_obs) else np.nan
            )
            return {
    ...
    def plot_drawdown(self):
        ...
        fig.add_trace(
            go.Scatter(
                y=self.drawdowns, mode="lines", name="Drawdown", line={"color": "red", "width": 2}
            )
        )
        fig.update_layout(
            title="Drawdown",
    ...
    def plot_returns(self):
        ...
        fig.add_trace(
            go.Scatter(
                y=self.returns, mode="lines", name="Returns", line={"color": "green", "width": 2}
            )
        )
        fig.update_layout(
            title="Returns",
    ...
    def plot_positions(self):
        ...
        fig = go.Figure(data=go.Heatmap(z=z.T, x=times, y=all_symbols, colorscale="RdBu", zmid=0))
        fig.update_layout(
